...
exports.createBooking = async (req, res) => {
  try {
    ...
    await sendEmail({
      email: user.email,
      subject: 'Enjoy Booking - Booking Confirmation',
      html: `
        <h1>Booking Confirmation</h1>
        <p>Thank you for your booking with Enjoy Booking!</p>
    ...
    await sendSMS({
      ...
      body: `Your booking with Enjoy Booking (ID: ${booking._id}) has been received and is pending payment. Total: ${payment.totalAmount}`
    });
    ...
    if (venue) {
      ...
      await sendEmail({
        ...
        subject: 'Enjoy Booking - New Booking Request',
        html: `
    ...
    if (services && services.length > 0) {
      for (const service of services) {
        ...
        await sendEmail({
          ...
          subject: 'Enjoy Booking - New Service Request',
          html: `
...
exports.updateBookingStatus = async (req, res) => {
  try {
    ...
    await sendEmail({
      email: user.email,
      subject: `Enjoy Booking - Booking ${status.charAt(0).toUpperCase() + status.slice(1)}`,
      html: `
        <h1>Booking ${status.charAt(0).toUpperCase() + status.slice(1)}</h1>
        <p>Your booking with Enjoy Booking has been ${status}.</p>
        <p>Booking ID: ${booking._id}</p>
    ...
    if (status === 'cancelled' || status === 'completed') {
      ...
      for (const service of booking.services) {
        ...
        await sendEmail({
          email: serviceProvider.email,
          subject: `Enjoy Booking - Service Request ${status.charAt(0).toUpperCase() + status.slice(1)}`,
          html: `
